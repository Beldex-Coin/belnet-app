...
if [ -n "$DRONE_TAG" ]; then
    ...
    base="belnet-flutter-$os-$DRONE_TAG"
else
    # Otherwise build a length name from the datetime and commit hash, such as:
    # belnet-linux-amd64-20200522T212342Z-04d7dcc54
    base="belnet-flutter-$os-$(date --date=@$DRONE_BUILD_CREATED +%Y%m%dT%H%M%SZ)-${DRONE_COMMIT:0:9}"
fi

mkdir -v "$base"
if [ -e build/app/outputs/flutter-apk/app-release.apk ]; then
    cp build/app/outputs/flutter-apk/app-release.apk "$base/belnet-flutter.apk"
    # zipit up yo
    archive="$base.zip"
    zip -r "$archive" "$base"
elif [ -e build/app/outputs/flutter-apk/app-debug.apk ]; then
    # ziiiiiip it
    cp build/app/outputs/flutter-apk/app-debug.apk "$base/belnet-flutter.apk"
    archive="$base.zip"
    zip -r "$archive" "$base"
else
